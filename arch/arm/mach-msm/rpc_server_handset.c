 ...
 */
static uint32_t m_key_code = 0;
static struct timer_list hs_rpc_timer;

static void hs_rpc_timer_func(unsigned long data)
{
	hs->mic_on = hs->hs_on = (m_key_code != HS_REL_K) ? 1 : 0;
	input_report_switch(hs->ipdev, SW_HEADPHONE_INSERT, hs->hs_on);
	input_report_switch(hs->ipdev, SW_MICROPHONE_INSERT, hs->mic_on);
	update_state();
	if (hs->mic_on)
	{
		printk("report_hs_key, SW_HEADPHONE_INSERT_W_MIC, insert\n");
	}
	else
	{
		printk("report_hs_key, SW_HEADPHONE_INSERT_W_MIC, out\n");
	}
}
...
static void report_hs_key(uint32_t key_code, uint32_t key_parm)
{
	...
	static unsigned long ul_insert_time = INITIAL_JIFFIES;
	...
	case KEY_VOLUMEUP:
	case KEY_VOLUMEDOWN:
		if (time_after(jiffies, ul_insert_time + HZ))
		{
			input_report_key(hs->ipdev, key, (key_code != HS_REL_K));
		}
		else
		{
			pr_debug("report_hs_key, key pressed too quickly\n");
		}
		break;
	case SW_HEADPHONE_INSERT_W_MIC:
        m_key_code = key_code;
        printk("SW_HEADPHONE_INSERT_W_MIC\n");
        if (m_key_code != HS_REL_K)
    	{
    		ul_insert_time = jiffies;
    	}
        mod_timer(&hs_rpc_timer, jiffies + msecs_to_jiffies(300));
		break;
	...
	case SW_HEADPHONE_INSERT:
		...
		if (hs->mic_on)
		{
			pr_debug("report_hs_key, SW_HEADPHONE_INSERT, insert\n");
			ul_insert_time = jiffies;
		}
		else
		{
			pr_debug("report_hs_key, SW_HEADPHONE_INSERT_W_MIC, out\n");
		}
		break;
	case SW_MICROPHONE_INSERT:
		...
		if (hs->mic_on)
		{
			pr_debug("report_hs_key, SW_MICROPHONE_INSERT, insert\n");
			ul_insert_time = jiffies;
		}
		else
		{
...
static int __devinit hs_rpc_init(void)
{
    ...
    setup_timer(&hs_rpc_timer, hs_rpc_timer_func, 0);
