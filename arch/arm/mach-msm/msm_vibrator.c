...
static struct hrtimer vibe_timer_second_stop;
...
static void vibrator_enable(struct timed_output_dev *dev, int value)
{
	...
	if (value == 0) {
		...
		hrtimer_start(&vibrator_timer,
			      ...
			      HRTIMER_MODE_REL);
		hrtimer_cancel(&vibe_timer_second_stop);
		hrtimer_start(&vibe_timer_second_stop,
			  	  ktime_set(1, 200*1000000),
			      HRTIMER_MODE_REL);
	}
...
void __init msm_init_pmic_vibrator(void)
{
	...
	hrtimer_init(&vibe_timer_second_stop, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	vibe_timer_second_stop.function = vibrator_timer_func;
