		...
		unsigned long arg)
{
	...
	case SND_SET_DEVICE:
		...
		/* +++ add for MID, MID has no HANDSET_SPKR/HANDSET_MIC, use SPKR_PHONE_MONO/SPKR_PHONE_MIC to replace it*/
		if (HANDSET_SPKR == dev.device.rx_device || HANDSET_MIC == dev.device.rx_device) {
			dev.device.rx_device = SPKR_PHONE_MONO;
		}
        if (HANDSET_MIC == dev.device.tx_device || HANDSET_SPKR == dev.device.tx_device) {
			dev.device.tx_device = SPKR_PHONE_MIC;
		}
		/* +++ add for MID, MID has no HANDSET_SPKR/HANDSET_MIC, use SPKR_PHONE_MONO/SPKR_PHONE_MIC to replace it*/
	...
	case SND_SET_VOLUME:
        ...
        /* +++ add for MID, MID has no HANDSET_SPKR/HANDSET_MIC, use SPKR_PHONE_MONO/SPKR_PHONE_MIC to replace it*/
		if (HANDSET_SPKR == vol.device.rx_device || HANDSET_MIC == vol.device.rx_device) {
			dev.device.rx_device = SPKR_PHONE_MONO;
		}
        if (HANDSET_MIC == vol.device.tx_device || HANDSET_SPKR == vol.device.tx_device) {
			dev.device.tx_device = SPKR_PHONE_MIC;
		}
		/* +++ add for MID, MID has no HANDSET_SPKR/HANDSET_MIC, use SPKR_PHONE_MONO/SPKR_PHONE_MIC to replace it*/
...
static long snd_cad_vol_enable(const char *arg)
{
    ...
    /* +++ add for MID, MID has no HANDSET_SPKR/HANDSET_MIC, use SPKR_PHONE_MONO/SPKR_PHONE_MIC to replace it*/
	if (HANDSET_SPKR == vol.device.rx_device || HANDSET_MIC == vol.device.rx_device) {
		vol.device.rx_device = SPKR_PHONE_MONO;
	}
    if (HANDSET_MIC == vol.device.tx_device || HANDSET_SPKR == vol.device.tx_device) {
		vol.device.tx_device = SPKR_PHONE_MIC;
	}
	/* +++ add for MID, MID has no HANDSET_SPKR/HANDSET_MIC, use SPKR_PHONE_MONO/SPKR_PHONE_MIC to replace it*/
