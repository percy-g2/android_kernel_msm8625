/*
	...
	History:
		...
		2012/04/02 input_report_key(android 4.0 event input)
...
#define ILI2113_TS_SUSPEND         gpio_set_value(TS_RESET_PORT, 0)
#define ILI2113_TS_WORK             gpio_set_value(TS_RESET_PORT, 1)
#define ILI2113_TS_SWAP(a, b) do {unsigned int temp; temp = a; a = b; b = temp;} while (0)
...
#define KEYPAD01_X1	30
#define KEYPAD01_X2	130
#define KEYPAD02_X1	190
#define KEYPAD02_X2	290
#define KEYPAD03_X1	330
#define KEYPAD03_X2	450
#define KEYPAD04_X1	3000
#define KEYPAD04_X2	3968
#define KEYPAD_Y	800
// definitions
#define ILITEK_I2C_RETRY_COUNT			3
#define ILITEK_I2C_DRIVER_NAME			"ili2113"
...
static int ilitek_i2c_read(struct i2c_client *client, uint8_t cmd, uint8_t *data, int length)
{
	...
	ret = ilitek_i2c_transfer(client, msgs_data, 1);
    if(ret < 0) {
...
static int ilitek_i2c_process_and_report(void)
{
    int i,j=0, len, ret, x=0, y=0,key,mult_tp_id;
            ...
            if(ilitek_i2c_read(i2c.client, ILITEK_TP_CMD_READ_SUB_DATA, buf, 5)) {
                ...
                */
                ...
                case RELEASE_POINT:
                    ...
                    */
            if(j==1){
			DBG("Release: eshion, ID=%02X, X=%04d, Y=%04d\n",buf[0]  & 0x3F, x,y);
...
ilitek_i2c_register_device(void)
{
    ...
    if(ret == 0) {
        ...
        // read touch parameter
      	...
      	if (ret == -1)
      	{
            printk(ILITEK_ERROR_LEVEL "%s, ilitek_i2c_read_tp_info error\n", __func__);
            return ret;
      	}
