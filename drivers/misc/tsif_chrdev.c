			 ...
			 loff_t *f_pos)
{
	...
	if (the_dev->ri != (the_dev->rptr / TSIF_PKT_SIZE)) {
		//dev_err(the_dev->dev, "%s: inconsistent read pointers: ri %d rptr %d\n", __func__, the_dev->ri, the_dev->rptr);
	...
	if (the_dev->ri == the_dev->wi) {
		...
		if (filp->f_flags & O_NONBLOCK) {
			if (the_dev->state == tsif_state_running) {
                //dev_err(the_dev->dev, "tsif_read: return -EAGAIN");
				return -EAGAIN;
			} else {
				/* not running -> EOF */
                //dev_err(the_dev->dev, "tsif_read: not running -> EOF, return 0");
				return 0;
			}
		}
		if (wait_event_interruptible_timeout(the_dev->wq_read,
		      (the_dev->ri != the_dev->wi) ||
		      (the_dev->state != tsif_state_running), HZ*1)) {
			/* got signal -> tell FS to handle it */
            //dev_err(the_dev->dev, "tsif_read: got signal -> tell FS to handle it, return -ERESTARTSYS");
			return -ERESTARTSYS;
		}
		if (the_dev->ri == the_dev->wi) {
			/* still no data -> EOF */
            //dev_err(the_dev->dev, "tsif_read: still no data -> EOF, return 0");
			return 0;
		}
	}
    ...
    tmprptr = the_dev->rptr;
    for (loop = 0; loop < (avail/TSIF_PKT_SIZE); loop++) {
        if (tmplen + 188 >= 4096) {
            break;
        }
        memcpy(tmpbuffer, the_dev->data_buffer + tmprptr, 188);
        tmprptr = tmprptr + TSIF_PKT_SIZE;
        tmpbuffer = tmpbuffer + 188;
        tmplen = tmplen + 188;
    }

    //dev_err(the_dev->dev, "tsif_read: avail:%d, TSIF_PKT_SIZE:%d [a/s:%d], buffer_len %d\n", avail, TSIF_PKT_SIZE, (avail/TSIF_PKT_SIZE), tmplen);
    
    if (copy_to_user(buf, tmp_buffer, tmplen))
        return -EFAULT;
#endif
