...
{
#if defined(RTV_IF_SPI) || defined(RTV_IF_EBI2) || defined(RTV_FIC_INTR_ENABLED)
...
void rtvOEM_PowerOn(int on)
{
	...
	if( on )
	{
		...
		pmic_gpio_set_value(MTV_PWR_PM_GPIO, 0);
		RTV_DELAY_MS(10);
		
		/* Set the GPIO of MTV_EN pin to high. */
		//	gpio_set_value(MTV_PWR_EN, 1);
		pmic_gpio_set_value(MTV_PWR_PM_GPIO, 1);
		RTV_DELAY_MS(20);	
