...
UINT rtvISDBT_GetAntennaLevel(U32 dwCNR)
{
	...
	static const UINT aAntLvlTbl[ISDBT_MAX_NUM_ANTENNA_LEVEL-1]
		/* Antenna Level :  6 , 5,  4, 3, 2, 1 */
//		= {150000, 120000, 110000,  90000, 70000, 40000};
		= {250000, 210000, 160000, 110000, 60000, 40000};
	do {
...
INT rtvISDBT_Initialize(E_RTV_COUNTRY_BAND_TYPE eRtvCountryBandType, UINT nThresholdSize)
{
	...
	if(nThresholdSize > (RTV_TS_PACKET_SIZE * 10))
	{
		RTV_DBGMSG0("[rtvISDBT_Initialize] Maximum TSP size must be less than (188 * 18) bytes\n");
		return RTV_INVAILD_THRESHOLD_SIZE;
	}

#if defined(RTV_IF_SERIAL_TSIF)
  g_nRtvMscThresholdSize = 188;
#else
