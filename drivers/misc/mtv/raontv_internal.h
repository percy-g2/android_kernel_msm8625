...
{	
	...
	switch( eTvMode )
	{
		...
		case RTV_TV_MODE_TDMB:
			...
			RTV_REG_SET(0x63, g_bRtvIntrMaskRegH|RE_CONFIG_E_INT); // RECONFIG
...
static INLINE void rtv_StreamRestore(E_RTV_TV_MODE_TYPE eTvMode)
{
	...
	switch( eTvMode )
	{
		...
		case RTV_TV_MODE_TDMB:
		    	...
		    	RTV_REG_MAP_SEL(HOST_PAGE);
		    	RTV_REG_SET(0x62, g_bRtvIntrMaskRegL); // FIC, MSC0, MSC1
		    	RTV_REG_SET(0x63, g_bRtvIntrMaskRegH); // RECONFIG
			...
			RTV_REG_SET(0x29, 0x00);	 
			  ...
			  RTV_REG_MAP_SEL(DD_PAGE);
			  RTV_REG_SET(INT_E_UCLRH, 0x04); // RECONFIG Interrupt status clear. 
...
static INLINE void rtv_CloseFIC(void)
{
	if(g_fDabOpenedFIC == FALSE)
		return;
	...
	g_fDabOpenedFIC = FALSE;
}

static INLINE void rtv_OpenFIC(void)
{	
	if(g_fDabOpenedFIC == TRUE)
		return;
		
	g_fDabOpenedFIC = TRUE;
