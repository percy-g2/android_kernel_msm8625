...
void msm_battery_update_psy_status(void)
{
	...
	else
	{
		...
		battery_capacity	= reply_charger.battery_capacity & 0x7F; //msm_battery_info.calculate_capacity(battery_voltage);
		//pr_debug("battery_voltage:%d, real:%d, capacity:%d\n", battery_voltage, battery_voltage_real,battery_capacity);
	...
	if (battery_capacity == 100) {
		msm_battery_info.psy_status = POWER_SUPPLY_STATUS_FULL;
	} else if (charger_status == CHARGER_STATUS_NULL || !is_charging) {
		msm_battery_info.psy_status = POWER_SUPPLY_STATUS_DISCHARGING;	
	} else {
		msm_battery_info.psy_status = POWER_SUPPLY_STATUS_CHARGING;
	}
