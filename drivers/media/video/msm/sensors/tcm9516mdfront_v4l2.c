...
#include "msm.h"
#define SENSOR_NAME "tcm9516mdfront"
#define PLATFORM_DRIVER_NAME "msm_camera_tcm9516mdfront"
#define tcm9516mdfront_obj tcm9516mdfront_##obj
...
#define TCM9516MDFRONT_VERBOSE_DGB

#ifdef TCM9516MDFRONT_VERBOSE_DGB
...
static struct msm_sensor_ctrl_t tcm9516mdfront_s_ctrl;
static int is_tcm9516mdfront_ac = 0;

DEFINE_MUTEX(tcm9516mdfront_mut);

static struct msm_camera_i2c_reg_conf tcm9516mdfront_start_settings[] = {
	{0x0100, 0x80},  /* streaming on */
};

static struct msm_camera_i2c_reg_conf tcm9516mdfront_stop_settings[] = {
	{0x0100, 0x80},  /* streaming off*/
};

static struct msm_camera_i2c_reg_conf tcm9516mdfront_groupon_settings[] = {
	{0x0100, 0x88},
};

static struct msm_camera_i2c_reg_conf tcm9516mdfront_groupoff_settings[] = {
	{0x0100, 0x80},
};

static struct msm_camera_i2c_reg_conf tcm9516mdfront_prev_settings[] = {
	//TAB_NAME: Preview-1280x720-29.1fps
//	{0x0100,0x88},//MODE_SEL/VREVON/HREVON/SWRST/GRHOLD/-/-/OUT_FORMAT;
	{0x0300,0x40},//-/PIXCKDIV[2:0]/-/SYSCKDIV[2:0];
	{0x0301,0x01},//-/-/-/-/-/-/PRECKDIV[1:0];  //0x01: 24MHz  0x02: 12Mhz
	{0x0302,0x01},//-/-/-/-/-/OPSYSDIV[2:0];
	{0x0303,0x00},//-/-/-/-/-/-/-/PLLMULT[8];
	{0x0304,0x35},//PLLMULT[7:0];
	
	{0x0305,0x06},//TTLINE[15:8];
	{0x0306,0xA4},//TTLINE[7:0];
	
	{0x0307,0x0D},//TTLDOT[15:8];
	{0x0308,0x48},//TTLDOT[7:0];
...
static struct msm_camera_i2c_reg_conf tcm9516mdfront_snap_settings[] = {
	//TAB_NAME: Capture-2608x1960-7.14fps
//	{0x0100,0x88},//MODE_SEL/VREVON/HREVON/SWRST/GRHOLD/-/-/OUT_FORMAT;
	{0x0300,0x40}, //4.7 fps //{0x0300,0x42}, 4 fps//{0x0300,0x40},//-/PIXCKDIV[2:0]/-/SYSCKDIV[2:0];
	{0x0301,0x01},//-/-/-/-/-/-/PRECKDIV[1:0];   //0x01: 24MHz  0x02: 12Mhz
	{0x0302,0x01},//{0x0302,0x01},//-/-/-/-/-/OPSYSDIV[2:0];
	{0x0303,0x00},//-/-/-/-/-/-/-/PLLMULT[8];
	{0x0304,0x41},//PLLMULT[7:0];
	{0x0305,0x07},//TTLINE[15:8];
	{0x0306,0xE8},//TTLINE[7:0];
	{0x0307,0x1A},//TTLDOT[15:8];
	{0x0308,0x58},//TTLDOT[7:0];
	{0x030D,0x00},//-/-/-/-/HOUT_STADR[11:8];
	{0x030E,0x00},//HOUT_STADR[7:0];
	{0x030F,0x00},//-/-/-/-/VOUT_STADR[11:8];
	{0x0310,0x00},//VOUT_STADR[7:0];
...
static struct msm_camera_i2c_reg_conf tcm9516mdfront_video_60fps_settings[] = {
	
};

static struct msm_camera_i2c_reg_conf tcm9516mdfront_video_90fps_settings[] = {
	
};

static struct msm_camera_i2c_reg_conf tcm9516mdfront_zsl_settings[] = {
	
};

static struct msm_camera_i2c_reg_conf tcm9516mdfront_recommend_settings[] = {
	//TAB_NAME: MIPID02
	//CAM1: 1, 2, 1, 10
	//REG_DAT:
	//TAB_NAME: Overall
	...
	{0x3270,0x70},//{0x3270,0x30},//(7)/ LNNARROW/ HLNRSW/ PIXDANSA/ HOBPSTN[3:0];
	...
	{0x3274,0x03},//{0x3274,0x00},//BBPCLV[7:0];
	{0x3275,0x04},//{0x3275,0x18},//WBPCLV[7:0];
...
{0x327A,0x00},//PWBGAINB[7:0];
	...
	{0x32C2,0x84},//REGVD_SEL[1:0]/ (5)/ (4)/ DLREN_SEL/ MIPI1L / AUTO_R_SEL/ LPFR_SEL;
...
static struct msm_camera_i2c_conf_array tcm9516mdfront_init_conf[] = {
	{&tcm9516mdfront_recommend_settings[0],
	ARRAY_SIZE(tcm9516mdfront_recommend_settings), 0, MSM_CAMERA_I2C_BYTE_DATA}
};

static struct msm_camera_i2c_conf_array tcm9516mdfront_confs[] = {
	{&tcm9516mdfront_snap_settings[0],
	ARRAY_SIZE(tcm9516mdfront_snap_settings), 0, MSM_CAMERA_I2C_BYTE_DATA},
	{&tcm9516mdfront_prev_settings[0],
	ARRAY_SIZE(tcm9516mdfront_prev_settings), 0, MSM_CAMERA_I2C_BYTE_DATA},
	{&tcm9516mdfront_video_60fps_settings[0],
	ARRAY_SIZE(tcm9516mdfront_video_60fps_settings), 0, MSM_CAMERA_I2C_BYTE_DATA},
	{&tcm9516mdfront_video_90fps_settings[0],
	ARRAY_SIZE(tcm9516mdfront_video_90fps_settings), 0, MSM_CAMERA_I2C_BYTE_DATA},
	{&tcm9516mdfront_zsl_settings[0],
	ARRAY_SIZE(tcm9516mdfront_zsl_settings), 0, MSM_CAMERA_I2C_BYTE_DATA},
};

static struct msm_camera_csi_params tcm9516mdfront_csi_params = {
...
static struct v4l2_subdev_info tcm9516mdfront_subdev_info[] = {
	{
...
static struct msm_sensor_output_info_t tcm9516mdfront_dimensions[] = {
	{ /* For Snapshot*/
		.x_output = 2608,//0xA30
		.y_output = 1960,//0x7A8
		.line_length_pclk = 0x1A58,
		.frame_length_lines = 0x07e8, 
		.vt_pixel_clk = 97500000, //24400000, //97500000, 
		.op_pixel_clk = 48000000, //16300000, //48000000,
		.binning_factor = 0,
	},
	{ /* For PREVIEW */
		.x_output = 0x518, //0x500,//1306
		.y_output = 0x3D4,//980
		.line_length_pclk = 0xD48, //0xAC0,//2752
		.frame_length_lines = 0x6A4, //0x4C9,//1225
...
static struct msm_sensor_output_reg_addr_t tcm9516mdfront_reg_addr = {
	.x_output = 0x0311,
	.y_output = 0x0313,
...
static struct msm_camera_csi_params *tcm9516mdfront_csi_params_array[] = {
	&tcm9516mdfront_csi_params, /* Snapshot */
	&tcm9516mdfront_csi_params, /* Preview */
	&tcm9516mdfront_csi_params, /* 60fps */
	&tcm9516mdfront_csi_params, /* 90fps */
	&tcm9516mdfront_csi_params, /* ZSL */
};

static struct msm_sensor_id_info_t tcm9516mdfront_id_info = {
	.sensor_id_reg_addr = 0x0000,
	.sensor_id = 0x1000,
};

static struct msm_sensor_exp_gain_info_t tcm9516mdfront_exp_gain_info = {
	.coarse_int_time_addr = 0x0200,
	.global_gain_addr = 0x0202,
	.vert_offset = 4,
};

void tcm9516mdfront_sensor_reset_stream(struct msm_sensor_ctrl_t *s_ctrl)
{
...
#define MSB 1
#define LSB 0
inline uint8_t tcm9516mdfront_byte(uint16_t word, uint8_t offset)
{
	return word >> (offset * BITS_PER_BYTE);
}

static int32_t tcm9516mdfront_write_pict_exp_gain(struct msm_sensor_ctrl_t *s_ctrl,uint16_t gain, uint32_t line)
{
#if 0
	uint16_t max_legal_gain = 0x0200;
	uint16_t min_ll_pck = 6744; //m9516mdfront_dimensions[0].line_length_pclk;  // From capture setting TTLDOT
...
//	tcm9516mdfront_dimensions[0].op_pixel_clk;
//	tcm9516mdfront_dimensions[0].line_length_pclk;
	...
	pr_info("tcm9516mdfront_write_pict_exp_gain fl_lines:%d fps_divider:%d Q10:%d \n", fl_lines, s_ctrl->fps_divider,Q10);
	pr_info("tcm9516mdfront_write_pict_exp_gain ll_pck:%d offset:%d \n", ll_pck, offset);
	pr_info("tcm9516mdfront_write_pict_exp_gain gain:%d line:%d \n", gain, line);
	...
	pr_info("tcm9516mdfront_write_pict_exp_gain gain_msb:%d,gain_lsb:%d,  intg_time_msb:%d, intg_time_lsb:%d, ll_pck_msb:%d, ll_pck_lsb:%d\n",gain_msb, gain_lsb, intg_time_msb, intg_time_lsb, ll_pck_msb, ll_pck_lsb);
...
/*	
	...
	pr_info("tcm9516mdfront_write_pict_exp_gain fl_lines:%d fps_divider:%d Q10:%d \n", fl_lines, s_ctrl->fps_divider,Q10);
	pr_info("tcm9516mdfront_write_pict_exp_gain offset:%d \n", offset);
	...
	vcount_msb = (uint8_t) ((vcount & 0xFF00) >> 8);
	vcount_lsb = (uint8_t) (vcount & 0x00FF);
	
	pr_info("tcm9516mdfront_write_pict_exp_gain gain_msb:%d,gain_lsb:%d,  intg_time_msb:%d, intg_time_lsb:%d, vcount_msb:%d, vcount_lsb:%d\n",gain_msb, gain_lsb, intg_time_msb, intg_time_lsb, vcount_msb, vcount_lsb);
...
static int32_t tcm9516mdfront_write_prev_exp_gain(struct msm_sensor_ctrl_t *s_ctrl, uint16_t gain, uint32_t line)
{
	uint16_t max_legal_gain = 0x0200;
	int32_t rc = 0;
	uint32_t fl_lines, offset;
	fl_lines = s_ctrl->curr_frame_length_lines;
	pr_info("tcm9516mdfront_write_prev_exp_gain :%d %d\n", gain, line);
	offset = s_ctrl->sensor_exp_gain_info->vert_offset;
	...
	msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
	s_ctrl->sensor_exp_gain_info->global_gain_addr,
	tcm9516mdfront_byte(gain, MSB),
	MSM_CAMERA_I2C_BYTE_DATA);
	msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
	s_ctrl->sensor_exp_gain_info->global_gain_addr + 1,
	tcm9516mdfront_byte(gain, LSB),
	MSM_CAMERA_I2C_BYTE_DATA);
	...
	if (line > (fl_lines - offset)) {
		...
		msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
		s_ctrl->sensor_output_reg_addr->frame_length_lines,
		tcm9516mdfront_byte(fl_lines, MSB),
		MSM_CAMERA_I2C_BYTE_DATA);
		msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
		s_ctrl->sensor_output_reg_addr->frame_length_lines + 1,
		tcm9516mdfront_byte(fl_lines, LSB),
		MSM_CAMERA_I2C_BYTE_DATA);
		/* Coarse Integration Time */
		msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
		s_ctrl->sensor_exp_gain_info->coarse_int_time_addr,
		tcm9516mdfront_byte(line, MSB),
		MSM_CAMERA_I2C_BYTE_DATA);
		msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
		s_ctrl->sensor_exp_gain_info->coarse_int_time_addr + 1,
		tcm9516mdfront_byte(line, LSB),
		MSM_CAMERA_I2C_BYTE_DATA);
		s_ctrl->func_tbl->sensor_group_hold_off(s_ctrl);
...
int32_t tcm9516mdfront_sensor_i2c_probe(struct i2c_client *client,
...
static const struct i2c_device_id tcm9516mdfront_i2c_id[] = {
	{SENSOR_NAME, (kernel_ulong_t)&tcm9516mdfront_s_ctrl},
	{ }
};

static struct i2c_driver tcm9516mdfront_i2c_driver = {
	.id_table = tcm9516mdfront_i2c_id,
	.probe  = tcm9516mdfront_sensor_i2c_probe,
	.driver = {
		.name = SENSOR_NAME,
	},
};

static struct msm_camera_i2c_client tcm9516mdfront_sensor_i2c_client = {
	.addr_type = MSM_CAMERA_I2C_WORD_ADDR,
};

static int __init msm_sensor_init_module(void)
{
	return i2c_add_driver(&tcm9516mdfront_i2c_driver);
}

static struct v4l2_subdev_core_ops tcm9516mdfront_subdev_core_ops = {
	.ioctl = msm_sensor_subdev_ioctl,
	.s_power = msm_sensor_power,
};

static struct v4l2_subdev_video_ops tcm9516mdfront_subdev_video_ops = {
	.enum_mbus_fmt = msm_sensor_v4l2_enum_fmt,
};

static struct v4l2_subdev_ops tcm9516mdfront_subdev_ops = {
	.core = &tcm9516mdfront_subdev_core_ops,
	.video  = &tcm9516mdfront_subdev_video_ops,
};

int32_t tcm9516mdfront_sensor_power_down(struct msm_sensor_ctrl_t *s_ctrl)
{
...
int32_t tcm9516mdfront_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	int32_t rc = 0;
	struct msm_camera_sensor_info *info = s_ctrl->sensordata;
	printk("##### %s: E\n",__func__);	
...
static void tcm9516mdfront_sensor_reset_pwd(struct msm_sensor_ctrl_t *s_ctrl)
{
	struct msm_camera_sensor_info *info = s_ctrl->sensordata;
...
int32_t tcm9516mdfront_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
  uint16_t id = 0;
  int32_t rc = -1;
  CDBG("tcm9516mdfront sensor read id\n");

  rc = msm_camera_i2c_read(s_ctrl->sensor_i2c_client,
    s_ctrl->sensor_id_info->sensor_id_reg_addr,
    &id, MSM_CAMERA_I2C_WORD_DATA);
  printk("tcm9516mdfront read_id 0x%x : 0x%x\n", s_ctrl->sensor_id_info->sensor_id_reg_addr, id);
  
  if (id != s_ctrl->sensor_id_info->sensor_id) {
    tcm9516mdfront_sensor_reset_pwd(s_ctrl);
	rc = msm_camera_i2c_read(s_ctrl->sensor_i2c_client,
    s_ctrl->sensor_id_info->sensor_id_reg_addr,
      &id, MSM_CAMERA_I2C_WORD_DATA);
    printk("tcm9516mdfront read_id 0x%x : 0x%x\n", s_ctrl->sensor_id_info->sensor_id_reg_addr, id);
    if (id == s_ctrl->sensor_id_info->sensor_id) is_tcm9516mdfront_ac = 1;
  }

  if (id != s_ctrl->sensor_id_info->sensor_id)
    return -ENODEV;

  CDBG("tcm9516mdfront readid ok, success\n");
  return rc;
}

static int32_t vfe_clk = 266667000;

int32_t tcm9516mdfront_sensor_setting(struct msm_sensor_ctrl_t *s_ctrl,
...
static struct msm_sensor_fn_t tcm9516mdfront_func_tbl = {
	.sensor_start_stream = msm_sensor_start_stream,
	.sensor_stop_stream = msm_sensor_stop_stream,
	.sensor_group_hold_on = msm_sensor_group_hold_on,
	.sensor_group_hold_off = msm_sensor_group_hold_off,
	.sensor_set_fps = msm_sensor_set_fps,
	.sensor_write_exp_gain = tcm9516mdfront_write_prev_exp_gain,
	.sensor_write_snapshot_exp_gain = tcm9516mdfront_write_pict_exp_gain,
	.sensor_csi_setting = tcm9516mdfront_sensor_setting,
	.sensor_set_sensor_mode = msm_sensor_set_sensor_mode,
	.sensor_mode_init = msm_sensor_mode_init,
	.sensor_get_output_info = msm_sensor_get_output_info,
	.sensor_config = msm_sensor_config,
	.sensor_power_up = tcm9516mdfront_sensor_power_up,
	.sensor_power_down = tcm9516mdfront_sensor_power_down,
	.sensor_match_id   = tcm9516mdfront_sensor_match_id,
};

static struct msm_sensor_reg_t tcm9516mdfront_regs = {
	.default_data_type = MSM_CAMERA_I2C_BYTE_DATA,
	.start_stream_conf = tcm9516mdfront_start_settings,
	.start_stream_conf_size = ARRAY_SIZE(tcm9516mdfront_start_settings),
	.stop_stream_conf = tcm9516mdfront_stop_settings,
	.stop_stream_conf_size = ARRAY_SIZE(tcm9516mdfront_stop_settings),
	.group_hold_on_conf = tcm9516mdfront_groupon_settings,
	.group_hold_on_conf_size = ARRAY_SIZE(tcm9516mdfront_groupon_settings),
	.group_hold_off_conf = tcm9516mdfront_groupoff_settings,
	.group_hold_off_conf_size =
		ARRAY_SIZE(tcm9516mdfront_groupoff_settings),
	.init_settings = &tcm9516mdfront_init_conf[0],
	.init_size = ARRAY_SIZE(tcm9516mdfront_init_conf),
	.mode_settings = &tcm9516mdfront_confs[0],
	.output_settings = &tcm9516mdfront_dimensions[0],
	.num_conf = ARRAY_SIZE(tcm9516mdfront_confs),
};

static struct msm_sensor_ctrl_t tcm9516mdfront_s_ctrl = {
	.msm_sensor_reg = &tcm9516mdfront_regs,
	.sensor_i2c_client = &tcm9516mdfront_sensor_i2c_client,
	.sensor_i2c_addr = 0x6c, 
	.sensor_output_reg_addr = &tcm9516mdfront_reg_addr,
	.sensor_id_info = &tcm9516mdfront_id_info,
	.sensor_exp_gain_info = &tcm9516mdfront_exp_gain_info,
	.cam_mode = MSM_SENSOR_MODE_INVALID,
	.csic_params = &tcm9516mdfront_csi_params_array[0],
	.msm_sensor_mutex = &tcm9516mdfront_mut,
	.sensor_i2c_driver = &tcm9516mdfront_i2c_driver,
	.sensor_v4l2_subdev_info = tcm9516mdfront_subdev_info,
	.sensor_v4l2_subdev_info_size = ARRAY_SIZE(tcm9516mdfront_subdev_info),
	.sensor_v4l2_subdev_ops = &tcm9516mdfront_subdev_ops,
	.func_tbl = &tcm9516mdfront_func_tbl,
