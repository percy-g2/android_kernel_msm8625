...
static struct msm_camera_i2c_reg_conf ov5647_snap_settings[] = {
	...
	//{0x3821, 0x06},
	//{0x3820, 0x00},
	{0x3820, 0x46},///0x3820, 0x41,Bit[2]:ISP vflip,Bit[1]:Sensor vflip
	{0x3821, 0x01},///0x3821, 0x07,Bit[2]:ISP mirror,Bit[1]:Sensor mirror
	{0x3612, 0x0b},
	{0x3618, 0x04},
	{0x380c, 0x0a},
...
static void ov5647_sensor_reset_pwd(struct msm_sensor_ctrl_t *s_ctrl)
{
	struct msm_camera_sensor_info *info = s_ctrl->sensordata;
	gpio_direction_output(info->sensor_pwd, 0);
	gpio_direction_output(info->sensor_reset, 0);
...
int32_t ov5647_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
  uint16_t id = 0;
  int32_t rc = -1;
  CDBG("ov5647 sensor read id\n");

  rc = msm_camera_i2c_read(s_ctrl->sensor_i2c_client,
    s_ctrl->sensor_id_info->sensor_id_reg_addr,
    &id, MSM_CAMERA_I2C_WORD_DATA);
  printk("ov5647 read_id 0x%x : 0x%x\n", s_ctrl->sensor_id_info->sensor_id_reg_addr, id);
  
  if (id != s_ctrl->sensor_id_info->sensor_id) {
    ov5647_sensor_reset_pwd(s_ctrl);
	rc = msm_camera_i2c_read(s_ctrl->sensor_i2c_client,
    s_ctrl->sensor_id_info->sensor_id_reg_addr,
      &id, MSM_CAMERA_I2C_WORD_DATA);
    printk("ov5647 read_id 0x%x : 0x%x\n", s_ctrl->sensor_id_info->sensor_id_reg_addr, id);
    if (id == s_ctrl->sensor_id_info->sensor_id) is_ov5647_ac = 1;
  }

  if (id != s_ctrl->sensor_id_info->sensor_id)
    return -ENODEV;

  CDBG("ov5647 readid ok, success\n");
  return rc;
}
...
static struct msm_sensor_fn_t ov5647_func_tbl = {
	...
	.sensor_match_id   = ov5647_sensor_match_id,
};
