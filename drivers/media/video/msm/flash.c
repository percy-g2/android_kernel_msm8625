...
static int msm_camera_set_flash_light(struct msm_camera_sensor_flash_led *led_src, unsigned led_state)
{
	enum led_brightness brightness;

	CDBG("msm_camera_set_flash_light state=%d,name=%s,name_len=%d\n", led_state, led_src->led_name, led_src->led_name_len);
	
	//Auto focus: MSM_CAMERA_LED_LOW -> MSM_CAMERA_LED_OFF
	//Capture: MSM_CAMERA_LED_LOW -> MSM_CAMERA_LED_HIGH -> MSM_CAMERA_LED_OFF
	if(led_state == MSM_CAMERA_LED_HIGH)brightness = LED_FULL;
		else if(led_state == MSM_CAMERA_LED_LOW)brightness = LED_HALF;//Jed, comments led control when doing auto-focus
	else brightness = LED_OFF;

	led_brightness_set_ex(led_src->led_name, led_src->led_name_len, brightness, led_src->delay_on);
	return 0;
}
	...
	struct msm_camera_sensor_flash_data *fdata, unsigned led_state)
{
	...
	CDBG("msm_camera_flash_set_led_state,flash_type=%d,led_sstate:%d \n",fdata->flash_type, led_state);

	if (fdata->flash_type != MSM_CAMERA_FLASH_LED ||
		fdata->flash_src == NULL)
		return -ENODEV;

    CDBG("msm_camera_flash_set_led_state, flash_sr_type=%d\n",fdata->flash_src->flash_sr_type);
	...
	case MSM_CAMERA_FLASH_SRC_LED:
		rc = msm_camera_set_flash_light(
				&fdata->flash_src->_fsrc.led_src,
				led_state);
		break;
	...
	struct flash_ctrl_data *flash_info)
{
	int rc = 0;
	sensor_data = sdata;
	CDBG("msm_flash_ctrl,flashtype=%d,led_state=%d\n",flash_info->flashtype,flash_info->ctrl_data.led_state);
