	...
	void __user *arg)
{
	...
	info.af_enable = sdata->af_enable;//Jed, support YUV AF, BTW RAW AF is fixed
	printk("------msm_ctrl %s: af_enable %d\n", __func__, info.af_enable);
				 ...
				 const char *const apps_id)
{
	...
	if (!p_mctl->opencnt) {
		...
		printk("msm_mctl_open enter, csid_core=%d\n",csid_core);
		...
		if (p_mctl->csiphy_sdev) {
			printk("msm_mctl_open enter, VIDIOC_MSM_CSIPHY_INIT\n");
		...
		if (p_mctl->csid_sdev) {
			printk("msm_mctl_open enter, VIDIOC_MSM_CSID_INIT\n");
		...
		if (p_mctl->csic_sdev) {
			printk("msm_mctl_open enter, VIDIOC_MSM_CSIC_INIT\n");
