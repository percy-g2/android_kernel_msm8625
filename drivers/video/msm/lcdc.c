...
static struct clk *ebi_lcdc_clk = NULL;
...
static int lcdc_on(struct platform_device *pdev)
{
	...
	if (mfd->ebi1_clk) {
		clk_prepare(mfd->ebi1_clk);
		if (mdp_rev == MDP_REV_303) {
			//if (clk_set_rate(mfd->ebi1_clk, 65000000))
			if (clk_set_rate(mfd->ebi1_clk, 100000000))
				pr_err("%s: ebi1_lcdc_clk set rate failed\n",__func__);
		} else {
...
static int lcdc_probe(struct platform_device *pdev)
{
...
#ifndef CONFIG_MSM_BUS_SCALING
		ebi_lcdc_clk = clk_get(&pdev->dev, "mem_clk");
		if (IS_ERR(ebi_lcdc_clk)){
			pr_err("getting clk failed for lcdc(mem_clk).\n");
			return PTR_ERR(ebi_lcdc_clk);
		}
#endif
	...
	mfd = platform_get_drvdata(pdev);
	if(ebi_lcdc_clk)
		mfd->ebi1_clk = ebi_lcdc_clk;
	else
		mfd->ebi1_clk = clk_get(&pdev->dev, "mem_clk");// ebi1_clk;
	...
	pr_err("lcdc_probe succ exit, id=%d.\n", pdev->id);
	return 0;

lcdc_probe_err:
	platform_device_put(mdp_dev);
	pr_err("lcdc_probe fail exit, id=%d.\n", pdev->id);
	return rc;
}
