...
int mdp_dsi_video_on(struct platform_device *pdev)
{
	...
	//src_rgb565+dst_r888=0x2000000+0x180000+0x2100+0x3f=0x218213f
	dma2_cfg_reg = DMA_PACK_ALIGN_LSB | DMA_OUT_SEL_DSI_VIDEO;//= 0x0 | (3<<19) = 0x180000

	if (mfd->fb_imgType == MDP_BGR_565)
		dma2_cfg_reg |= DMA_PACK_PATTERN_BGR;//|=((0x01<<4)|(0x00<<2)|0x02) <<8)=0x1200
	else if (mfd->fb_imgType == MDP_RGBA_8888)
		dma2_cfg_reg |= DMA_PACK_PATTERN_BGR;
	else
		dma2_cfg_reg |= DMA_PACK_PATTERN_RGB;//|((0x02<<4)|(0x00<<2)|0x01) <<8)=0x2100

	if (bpp == 2)
		dma2_cfg_reg |= DMA_IBUF_FORMAT_RGB565;//|=0x2000000
	else if (bpp == 3)
		dma2_cfg_reg |= DMA_IBUF_FORMAT_RGB888;//|=0x0000000
	else
		dma2_cfg_reg |= DMA_IBUF_FORMAT_xRGB8888_OR_ARGB8888;

	switch (mfd->panel_info.bpp) {
	case 24://|= (3<<4)|(3<<2)|(3<<0)=0x3f
		dma2_cfg_reg |= DMA_DSTC0G_8BITS |
			DMA_DSTC1B_8BITS | DMA_DSTC2R_8BITS;
		break;
	case 18://|= (2<<4)|(2<<2)|(2<<0)=0x2a
		dma2_cfg_reg |= DMA_DSTC0G_6BITS |
			DMA_DSTC1B_6BITS | DMA_DSTC2R_6BITS;
		break;
	case 16://|= (1<<4)|(1<<2)|(1<<0)=0x15
		dma2_cfg_reg |= DMA_DSTC0G_6BITS |
